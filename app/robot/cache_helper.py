...
from app.api import teams, disciplines, campi, semesters
...
class CacheHelper(KeyGenerator, object):
    def update_teams_index(self, modified_teams, excluded_teams, campus, semester):
        ...
        for excluded_team in excluded_teams:
            ...
            teams.get_team(excluded_team, overwrite=True, exclude=True)
    ...
    def update_disciplines_index(self, campus, semester, modified_disciplines, excluded_disciplines):
        ...
        for excluded_discipline in excluded_disciplines:
            ...
            discipline = disciplines.get_discipline(excluded_discipline)
            self.update_teams_index([], map(lambda team: str(team.id()), discipline.teams), campus, semester)
            disciplines.get_discipline(excluded_discipline, overwrite=True, exclude=True)
    ...
    def update_campi_cache(self, modified_campi, semester):
        ...
        campi_models = campi.get_campi({"semester": semester_key})
        new_campi = []
        modified = []
        for campus in campi_models:
        ...
        campi.get_campi({"semester": semester_key}, overwrite=True, update_with=new_campi)
        modified_campi = []
        return modified_campi

    def update_semester_cache(self, semester_key):
        logging.debug("Updating semesters cache")
        semesters_models = semesters.get_semesters({})
        new_semesters = []
        found = False
        for semester in semesters_models:
            if not semester:
                continue
        ...
        semesters.get_semesters({}, overwrite=True, update_with=new_semesters)

    def check_cache_existence(self, team, campus, semester):
        logging.warn("Checking cache existence")
        disciplines_results = disciplines.get_disciplines({
            "campus": self.generate_campus_key(campus, semester),
        ...
        disciplines_teams_results = teams.get_disciplines_teams({
